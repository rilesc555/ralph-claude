//! Main run loop and iteration delay handling.
//!
//! This module contains the core event loop for the Ralph TUI, handling
//! keyboard input, terminal rendering, PTY interaction, and iteration
//! state management.
...
use crate::theme::{
    ...
    CYAN_PRIMARY, GREEN_ACTIVE, GREEN_SUCCESS, RED_ERROR, TEXT_MUTED, TEXT_PRIMARY, TEXT_SECONDARY,
};
use crate::ui::{
    render_progress_cards, render_stat_cards, render_story_card, render_vt100_screen, wrap_text,
};
use crate::utils::format_duration;
use crate::watcher::build_ralph_prompt;

/// Main UI run loop for a single iteration.
///
/// Handles rendering, input processing, and state detection. Returns when:
/// - The child process exits
/// - A stop hook is detected in the output
/// - The user quits with Ctrl+Q
pub fn run(
    ...
    loop {
        ...
        if app.last_animation_update.elapsed() >= Duration::from_millis(100) {
            ...
            app.last_animation_update = std::time::Instant::now();
        }
...
///
/// Shows a countdown in the UI and allows the user to quit with Ctrl+Q.
pub fn run_delay(
    terminal: &mut Terminal<CrosstermBackend<io::Stdout>>,
    app: &mut App,
    last_cols: &mut u16,
...
/// Main iteration loop that orchestrates Claude spawning and iteration management.
///
/// This function handles:
/// - Running UI iterations via `run()`
/// - Managing Claude process lifecycle (spawn, cleanup, respawn)
/// - Handling delay periods between iterations
/// - Detecting completion conditions
pub fn run_loop(
    terminal: &mut Terminal<CrosstermBackend<io::Stdout>>,
    app: &mut App,
    pty_rows: u16,
    ...
    loop {
        ...
        let run_result = run(terminal, app, &mut last_cols, &mut last_rows);
        ...
        match app.iteration_state {
            ...
            IterationState::NeedsRestart => {
                ...
                app.delay_start = Some(Instant::now());

                // Wait for 2 seconds (with UI updates)
                let delay_result = run_delay(terminal, app, &mut last_cols, &mut last_rows);
